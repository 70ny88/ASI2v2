...
class Products extends CI_Controller {
	...
	function __construct() { 
		...
		$this->load->library(array('pagination','cart','email', 'util')); // Util => application/libraries
...
//$config['base_url'] = base_url().'index.php/products/index'; //utilizar esta url si no la reconoce sin el index.php	
        ...
        $config['total_rows'] = $this->db->get('vwproductos')->num_rows();
	...
	$data['title'] = 'Servicios Virtuales Altuve'; 
...
function add(){ 
	...
	$cantidad = $this->input->post('cantidad'); // La cantidad enviada
	
	if(!$cantidad)
	   $cantidad = 1; // si no selleciona alguna cantidad se le asingna 1
...
if($cart = $this->cart->contents()){ // verificamos si el carrito existe 
	...
	foreach($cart as $item){ //foreach contenedor
			...
			if($item['id'] === $id && !$product->valores){
				 ...
				 $row = $item['rowid']."-".($item['qty']+$cantidad);
...
$this->table->set_heading('Portatil', 'Detalle','Cantidad','Precio', 'Total'); // la tabla que irá en el correo
 ...
 $this->table->add_row('Total', '', '', '', $this->cart->format_number($this->cart->total()));
    ...
    $config = array('protocol'=>'smtp', 
	                ...
	                'smtp_user'=>'ghelena2114@gmail.com', // colocamos nuestra propia cuenta de Gmail
